	...
	void SolidFillPipeline::CreatePipeline()
	{
		{// Vertex shader
			bool success = Importer::CompileShaderToSPV(
				Path::Get("shaders/solid_fill_pipeline/SolidFillPipeline.vert.hlsl"),
				Path::Get("shaders/solid_fill_pipeline/SolidFillPipeline.vert.spv"),
				"vert"
			);
		...
		{// Fragment shader
			bool success = Importer::CompileShaderToSPV(
				Path::Get("shaders/solid_fill_pipeline/SolidFillPipeline.frag.hlsl"),
				Path::Get("shaders/solid_fill_pipeline/SolidFillPipeline.frag.spv"),
				"frag"
			);
			MFA_ASSERT(success == true);
		}
		...
		pipelineOptions.rasterizationSamples = LogicalDevice::GetMaxSampleCount();            // TODO Find a way to set sample count to 1. We only need MSAA for pbr-pipeline
